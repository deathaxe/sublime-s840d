...
class ToolTipCommand(sublime_plugin.EventListener):
  ...
  def on_hover(self, view, point, hover_zone):
    if hover_zone == sublime.HOVER_TEXT:
      self.run(view, point, view.word(point))


  def run(self, view, point, region):
...
def get_var_desc(word, lang):
  ...
  try:
    ...
    with sqlite3.connect(doc_cache) as sql:
      ...
      # data type
      if row[2]:
        tip_html += '<p><b>type:</b> ' + html_encode(row[2]) + '</p>'

      # description
      tip_html += html_encode(gzip.decompress(bytearray(row[5])).decode())
...
def package_name():
  """
  This function returns the package name without path or extension.
...
def cache_get_langs(sql):
  return [row[0] for row in sql.execute("SELECT DISTINCT lang FROM vars").fetchall()]

def cache_add_resources(sql, filter):
  """
  Loads documentation from sublime-package.
  ...
  for fn in sublime.find_resources(filter):
    ...
    if len(lang) == 2:
      try:
        ...
        cache_add_xml(sql,
                      xml.parseString(sublime.load_binary_resource(fn)),
                      lang)
      ...
      if s_name and (s_brief or s_desc):
        ...
        try:
          ...
          if s_name[:3] in ("$MA", "$SA"):
            # axis machine data's dim attributes don't store axis index
            # so we need to correct this here to make the variable behave
            # like any other.
            i_dim += 1
...
def print_vars():
  ...
  with sqlite3.connect(doc_cache) as sql:
    ...
    with open("C:\\Data\\cmc_doc_md.tea", "w") as out:
    ...
    with open("C:\\Data\\cmc_doc_vars.tea", "w") as out:
      for row in sql.execute("""
